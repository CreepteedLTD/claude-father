...
1. **Claude Code version** — needs ≥ 2.1.232 for cross-session messaging and channels. Check `claude --version`; if older, run `claude update` and tell the user existing sessions keep the old version until restarted.
   - **Marketplace hygiene**: ensure `~/.claude/settings.json` → `extraKnownMarketplaces.claude-father` is `{"source": {"source": "github", "repo": "CreepteedLTD/claude-father"}, "autoUpdate": true}` so every machine pulls plugin fixes automatically at startup. Exception: if the existing entry points at a local directory that is a git working copy the user develops in, leave it (that's the author's machine) but add `"autoUpdate": true` to it.
