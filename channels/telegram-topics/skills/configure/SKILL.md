...
# /claude-father:configure — Telegram Channel Setup
...
3. **What next** — end with a concrete next step based on state:
   - No token → *"Run `/claude-father:configure <token>` with the token from
     BotFather."*
   - Token set, policy is pairing, nobody allowed → *"DM your bot on
     Telegram. It replies with a code; approve with `/claude-father:access pair
     <code>`."*
...
3. **If yes and policy is still `pairing`** → *"Good. Let's lock it down so
   ...
   `/claude-father:access policy allowlist`. Do this proactively — don't wait to
   be asked.
4. **If no, people are missing** → *"Have them DM the bot; you'll approve
   each with `/claude-father:access pair <code>`. Run this skill again once
...
6. **If policy is already `allowlist`** → confirm this is the locked state.
   ...
   `/claude-father:access policy pairing` → they DM → you pair → flip back."*
...
- `access.json` is re-read on every inbound message — policy changes via
  `/claude-father:access` take effect immediately, no restart.
